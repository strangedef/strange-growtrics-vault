# API Testing — Technical Design (Tier 0 + Tier 1)

**Status:** Draft — brainstormed design, pending write-up review
**Scope:** Tier 0 (Foundation) and Tier 1 (v1, client-usable), per `api-testing-capability-spec.pdf` §5 tiering table
...
- For each **safe/idempotent endpoint**:
  ...
  5. **Generate a human-readable `name`** summarizing what the test case verifies (method + path + the notable assertions), e.g. `"GET /orders/{id} — returns valid order with positive total"` — so a client scanning a suite or a failure list understands intent without opening the test case. Auto-generated at creation time, editable by the user afterward.
- The `TestSuite` itself also gets an auto-generated `name` (e.g. `"{Target.name} — Suite v{version}"`), likewise editable.
...
| **TestSuite** | Versioned generated suite | `id`, `target_id`, `name` (auto-generated, editable), `spec_artifact_version`, `knowledge_artifact_version`, `version`, `test_cases[]` |
| **TestCase** | One generated request + assertions | `id`, `suite_id`, `name` (auto-generated, editable), `endpoint_ref`, `request_template`, `assertions[]` (each with rationale), `needs_seed_value: bool`, *(reserved, unused in Tier 0: `depends_on`, `capture`)* |
...
```json
{
  "id": "suite_01e1",
  "target_id": "tgt_01h9x",
  "name": "Modun Orders API — Suite v1",
...
```json
{
  "id": "tc_01f1",
  "suite_id": "suite_01e1",
  "name": "GET /orders/{id} — returns valid order with positive total",
...
# Tier 1 — v1, Client-Usable Design

## 9. Tier 1 Scope

### In scope
- Deepens testing on the **same GET/HEAD surface** Tier 0 already generates against — no new HTTP methods, no request chaining (both still Tier 2).
- **Schema/contract validation**: recursive, nested-structure-aware conformance checking (arrays of objects, objects within objects), not just top-level field presence.
- **Negative/edge-case fuzzing**: property-based, schema-driven — systematically mutates each parameter against its own declared constraint (wrong type, out-of-range, invalid enum, oversized string, missing required param), expecting the API to fail safely (400/422), not crash or leak data.
- **Auth/authz matrix**: no-creds, invalid-creds, expired-creds, wrong-scope-creds checks per endpoint, plus BOLA/BFLA cross-tenant replay — the latter two conditional on the client supplying extra auth identities.
- **Pagination checks**: auto-detected from spec heuristics (param names, response shape), generating boundary-page and metadata-consistency test cases wherever confidently detected.
- **Per-endpoint SLA**: client-configurable latency threshold per endpoint, checked as a single-request latency comparison (not load testing).
- **Per-category reporting**: test cases are now tagged by capability (`functional`, `schema`, `negative`, `auth`, `pagination`, `sla`), so results and skip notices can be read per category.

### Out of scope (deferred)
- Mutating methods, chained CRUD lifecycle generation — **Tier 2**.
- Coverage attribution *over time* (historical trending of what's tested) — **Tier 2**; Tier 1 only surfaces skip reasons for the current suite, not trends across suites.
- Real load/performance testing (p95/p99 under concurrent traffic) — not scoped to any tier yet; flagged as a distinct future capability if a client needs it.
- OWASP API Top 10 mapping, idempotency/retry-safety, rate-limiting, consumer-driven contract testing — **Tier 3**.

## 10. Architecture & Data Model Changes

Tier 1 doesn't change the five-stage pipeline shape from Tier 0 — it deepens Stage 1 (preserve nested schema), Stage 3 (generate more, categorized test cases), and Stage 4 (auth resolution per identity, request dedup). Three entity changes:

- **`Environment.identities[]`** (new, optional): a list of `AuthIdentity` records, each `{id, environment_id, label, kind, auth_config_ref}`. `kind` is one of `secondary_tenant | expired | limited_scope` — this generalizes what was originally scoped as "a second identity for BOLA" into a reusable slot for any auth-matrix variant that needs real credentials (BOLA needs `secondary_tenant`; the expired-token check needs `kind: expired`; the wrong-scope check needs `kind: limited_scope`). Each sub-test only generates if a matching `kind` is registered; otherwise it's skipped with an inline note — same graceful-degradation pattern as before, now applied consistently across the whole auth matrix, not just BOLA.
- **`TestCase.category`** (new): `functional | schema | negative | auth | pagination | sla`. `TestCase.assertions[].field` now supports array-wildcard path expressions (e.g. `line_items[].quantity`) to address nested/repeated structures.
- **`SLAConfig`** (new entity): `{id, target_id, endpoint_ref, max_latency_ms, set_by, updated_at}` — client-set per endpoint; an endpoint with no `SLAConfig` simply gets no `sla`-category test case (no hardcoded default, per the spec's explicit stance on this).
- **`TestSuite.generation_notes[]`** (new, lightweight): plain strings recording what was skipped and why for this suite version (e.g. `"BOLA/BFLA skipped for GET /orders/{id}: no secondary_tenant identity registered"`). Deliberately not a first-class trending entity — that's Tier 2's coverage-attribution work.

## 11. Stage-by-Stage Additions

### Stage 1 — Spec Normalization (extended)
- Must now preserve the **full nested response schema tree** (objects within objects, arrays of objects) rather than a flattened field list — required for Stage 3's recursive schema-conformance generation.

### Stage 3 — Generation (extended per category)
- **`schema`**: recursively walks the response schema tree; for every leaf field at any depth, generates a presence + type assertion using array-wildcard path notation for list items (e.g. `line_items[].id`, `line_items[].quantity`). Applies uniformly regardless of nesting depth — no special-casing per level.
- **`negative`**: for each parameter (path/query), mutates it against its own schema constraint — wrong type, boundary value, invalid enum, oversized string, omitted-if-required — and asserts the response is a safe 4xx, not a 5xx or malformed body.
- **`auth`**: generates no-creds and invalid-creds test cases unconditionally (synthesizable from the primary identity alone); generates expired-creds and wrong-scope-creds test cases only if a matching-`kind` identity is registered; generates BOLA/BFLA cross-identity test cases only if a `secondary_tenant` identity is registered. Any un-generated case is recorded in `TestSuite.generation_notes`.
- **`pagination`**: detects list endpoints via heuristics (query params named `page`/`limit`/`offset`/`cursor`/`size`, response shape wrapping an array with count/next metadata). Confident detections generate boundary-page (first, last, beyond-last) and metadata-consistency (no dupes/gaps, correct `hasNext`) test cases. Ambiguous ones are skipped with a `generation_notes` entry, not guessed.
- **`sla`**: for each endpoint with a client-set `SLAConfig`, generates a test case asserting `latency_ms <= max_latency_ms` on the endpoint's default request.

### Stage 4 — Execution (extended)
- Resolves auth **per identity** referenced by a test case, not just the environment's primary credentials.
- **Request deduplication**: when `functional`, `schema`, and `sla` test cases for the same endpoint would issue an identical request (same method/path/params/identity), Stage 4 issues it once and evaluates all three categories' assertions against the single captured response — an execution-layer optimization, transparent to the data model and to reporting. `negative`, `auth`, and `pagination` test cases always issue their own distinct requests, since their parameters or identity differ from the default case.

### Stage 5 — Reporting (extended)
- Findings now carry `category`, so results are readable per capability, not just per endpoint.
- `TestSuite.generation_notes` surfaces alongside the run report so a client sees not just pass/fail, but what wasn't tested and why, for that suite version.

## 12. Example Data (Tier 1 additions)

Continuing the Modun Orders API example from Tier 0 — `GET /orders/{id}` now returns nested `line_items`, and a list endpoint `GET /orders` is added.

**Nested response schema** (relevant excerpt of `SpecArtifact.canonical_endpoints` for `GET /orders/{id}`)
```json
{
  "method": "GET",
  "path": "/orders/{id}",
...
**AuthIdentity** (three, under the staging Environment)
```json
[
  { "id": "id_01k1", "environment_id": "env_01a1", "label": "Primary tenant", "kind": "primary", "auth_config_ref": { "type": "bearer", "vault_ref": "vault://modun/staging-token" } },
  { "id": "id_01k2", "environment_id": "env_01a1", "label": "Secondary tenant (test account B)", "kind": "secondary_tenant", "auth_config_ref": { "type": "bearer", "vault_ref": "vault://modun/staging-token-b" } },
  { "id": "id_01k3", "environment_id": "env_01a1", "label": "Read-only scoped token", "kind": "limited_scope", "auth_config_ref": { "type": "bearer", "vault_ref": "vault://modun/staging-token-readonly" } }
]
```
...
```json
{
  ...
  "endpoint_ref": "GET /orders/{id}",
  "max_latency_ms": 500,
  "set_by": "hoang.nguyen@growtrics.ai",
  "updated_at": "2026-07-20T09:00:00Z"
}
```

**TestCase — schema category** (nested assertions)
```json
{
  "id": "tc_02a1",
  "suite_id": "suite_01e2",
  ...
  "endpoint_ref": "GET /orders/{id}",
  "assertions": [
    { "type": "field_exists", "field": "line_items", "rationale": "spec: array field" },
    { "type": "field_type", "field": "line_items[].id", "expected": "string", "rationale": "spec: line_items[].id" },
    { "type": "field_type", "field": "line_items[].quantity", "expected": "number", "rationale": "spec: line_items[].quantity" },
    { "type": "field_type", "field": "line_items[].height", "expected": "number", "rationale": "spec: line_items[].height" },
    { "type": "field_type", "field": "line_items[].weight", "expected": "number", "rationale": "spec: line_items[].weight" }
  ]
}
```

**RunResult for the schema test case** (item-level failure evidence)
```json
{
  "id": "res_02b1",
  "suite_run_id": "run_02c1",
  "test_case_id": "tc_02a1",
  "http_status": 200,
  "latency_ms": 168,
  "assertion_results": [
    { "type": "field_exists", "field": "line_items", "passed": true },
    { "type": "field_type", "field": "line_items[].id", "passed": true },
    { "type": "field_type", "field": "line_items[].quantity", "passed": false, "failing_indexes": [2], "detail": "line_items[2].quantity missing" },
    { "type": "field_type", "field": "line_items[].height", "passed": true },
    { "type": "field_type", "field": "line_items[].weight", "passed": true }
  ],
  "outcome": "fail"
}
```

**TestCase — negative category**
```json
{
  "id": "tc_02d1",
  "suite_id": "suite_01e2",
  "name": "GET /orders?limit=-1 — rejects invalid limit safely",
  "category": "negative",
  "endpoint_ref": "GET /orders",
  "request_template": { "query_params": { "limit": -1 } },
  "assertions": [
    { "type": "status_code", "expected": "4xx", "rationale": "negative: limit violates declared minimum(1)" }
  ]
}
```

**TestCase — auth category (BOLA)**
```json
{
  "id": "tc_02e1",
  "suite_id": "suite_01e2",
  "name": "GET /orders/{id} — secondary tenant cannot access primary tenant's order",
  "category": "auth",
  "endpoint_ref": "GET /orders/{id}",
  "request_template": { "path_params": { "id": "ord_8842" }, "identity_ref": "id_01k2" },
  "assertions": [
    { "type": "status_code", "expected": "403|404", "rationale": "BOLA: resource belongs to Primary tenant, requested as Secondary tenant" }
  ]
}
```

**TestCase — pagination category**
```json
{
  "id": "tc_02f1",
  "suite_id": "suite_01e2",
  "name": "GET /orders — last page has no next-page marker",
  "category": "pagination",
  "endpoint_ref": "GET /orders",
  "request_template": { "query_params": { "page": "{{last_page}}" } },
  "assertions": [
    { "type": "field_equals", "field": "has_next", "expected": false, "rationale": "pagination: last page metadata consistency" }
  ]
}
```

**TestCase — sla category**
```json
{
  "id": "tc_02g1",
  "suite_id": "suite_01e2",
  "name": "GET /orders/{id} — responds within 500ms",
  "category": "sla",
  "endpoint_ref": "GET /orders/{id}",
  "assertions": [
    { "type": "latency_under", "expected_ms": 500, "rationale": "SLAConfig sla_01l1" }
  ]
}
```

**TestSuite.generation_notes** (example, for a target with only a `secondary_tenant` identity registered, no `limited_scope`)
```json
[
  "wrong-scope-creds check skipped for all endpoints: no limited_scope identity registered",
  "pagination skipped for GET /webhooks/events: ambiguous — no recognizable page/limit/cursor params in spec"
]
```

## 13. Key Design Decisions (Tier 1)

| Decision | Choice | Rationale |
|---|---|---|
| BOLA/BFLA mechanism | Multiple named `AuthIdentity` records per environment, conditional generation | Strongest bidirectional signal; generalized into a reusable `kind`-tagged slot for the rest of the auth matrix (expired, limited-scope), not a BOLA-only mechanism |
| Missing identities | Graceful degradation — skip + explicit inline note, no fallback mechanism | Keeps Tier 1 to one code path; matches the spec's "what we did NOT test and why" trust principle |
| Fuzzing strategy | Property-based, schema-driven mutation | Scales automatically with the spec, no payload library to maintain |
| Schema/negative/sla test cases | Separate `category`-tagged test cases per capability, not merged into the functional test case | Enables per-capability coverage reporting; Tier 2's coverage-attribution work builds directly on this |
| Schema conformance depth | Recursive tree walk with array-wildcard path assertions | Nested structures (e.g. `line_items[]`) need per-item, per-field checks, not just top-level presence |
| Pagination detection | Auto-detected via heuristics, ambiguous cases skipped not guessed | Pagination conventions vary too much to safely assume; a wrong guess produces a meaningless test |
| SLA testing | Single-request latency check against a client-set threshold | Matches the spec's "contained scope, real value fast" framing; true load testing is a distinct, larger capability not scoped here |
| Coverage gaps | Lightweight `generation_notes[]` strings on `TestSuite`, not a dedicated tracking entity | Full coverage-attribution-over-time is explicitly Tier 2 scope |
| Duplicate requests across categories | Deduped at execution time when request is identical (functional/schema/sla) | Avoids redundant load on the client's API without changing the data model or reporting |
