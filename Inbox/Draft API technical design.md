# Tier 0
Here's each stage broken down into concrete operations.
...
- **Compute a structural diff** against the target's previous SpecArtifact version, if one exists (added/removed/changed endpoints) — Tier 0 doesn't _act_ on this diff, but computing and storing it now means Tier 2's change-detection engine has data to consume from day one instead of retrofitting it.
...
- For each **safe/idempotent endpoint**:
    1. Build the request: path/query param values sourced from spec-provided examples where present; otherwise synthesized by type (string sample, valid enum value, etc.).
    2. **Base assertions**: expect 2xx; validate required response fields exist and match declared type.
    3. **Knowledge-informed assertions**: match extracted invariants to response fields by name/semantic similarity (an invariant about "prices are positive" attaches to fields like `price`/`amount`) → adds a _value_-correctness assertion, not just shape.
...
The `needs-seed-value` gap in Stage 3 is the one piece I'd flag as a real open design decision rather than a settled detail. Want me to address it now, or note it and move to the data model?